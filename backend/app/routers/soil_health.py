...
async def get_fertilizer_recommendation(
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Model not available, using mock prediction: {str(e)}")
            use_model = False
        ...
        if use_model:
            ...
            # Mock prediction based on nutrient levels
            if request.nitrogen < 40:
                ...
                prediction = 0  # 10-26-26
                confidence = 0.78
            else:
                prediction = 2  # 17-17-17
                confidence = 0.82
        ...
        guidelines = generate_application_guidelines_old(fertilizer_name)
...
@router.post("/fertilizer-prediction", response_model=FertilizerPredictionResponse)
